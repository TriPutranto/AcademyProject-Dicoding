...
import com.triputranto.academy.utils.LiveDataTestUtil
...
class AcademyRepositoryTest {
    ...
    fun getBookmarkedCourses() {
        val dummyCourses = MutableLiveData<List<CourseEntity>>()
        dummyCourses.value = DataDummy.generateDummyCourses()
        `when`(local.getAllBookmarkedCourse()).thenReturn(dummyCourses)

        val courseEntities = LiveDataTestUtil.getValue(academyRepository.getBookmarkedCourses())
        verify(local).getAllBookmarkedCourse()
    ...
    fun getCourseWithModules() {
        ...
        `when`(local.getCoursesWithModules(courseId)).thenReturn(dummyEntity)
        val courseEntities =
            LiveDataTestUtil.getValue(academyRepository.getCourseWithModules(courseId))
        verify(local).getCoursesWithModules(courseId)
        assertNotNull(courseEntities.data)
